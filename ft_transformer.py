...
class Tokenizer(nn.Module):
    ...
    def forward(self, x_num: Tensor, x_cat: ty.Optional[Tensor]) -> Tensor:
        ...
        x_num = torch.cat(
            [torch.ones(x_some.shape[0], 1, device=x_some.device)]  # [CLS] concat在了开头位置
...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        for tensor in [q, k, v]:
            if tensor.shape[-1] % self.n_heads != 0:
                raise ValueError("Input tensor's last dimension must be divisible by n_heads.")
        ...
        q = self._reshape(q)
        k = self._reshape(k)
        attention = F.softmax(q @ k.transpose(1, 2) / d_head_key ** 0.5, dim=-1)
...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        *,
        # tokenizer
        input_size: int,  # replace d_numerical
        categories: ty.Optional[ty.List[int]],
        token_bias: bool,
        # transformer
        n_layers: int,
        ...
        super().__init__()
        self.tokenizer = Tokenizer(input_size, categories, d_token, token_bias)
    ...
    def forward(self, x_num: Tensor) -> Tensor:  # disable x_cat
        x = self.tokenizer(x_num, x_cat=None)
        ...
        if x.shape[1] != 1:
            raise ValueError(f"wrong {x.shape=}, expect x.shape[1] == 1")
        x = x[:, 0, :]  # 0 is CLS token  # (batch_size, d_token)
        if self.last_normalization is not None:  # will triger TracerWarning in onnx.export, but it is fine to treat it as constant for a given model
            x = self.last_normalization(x)
        x = self.last_activation(x)
        x = self.head(x)
        x = x.squeeze()  # 原本的squeeze(-1) somehow会让onnx的输出的形状比squeeze()多一个dummy维
        return x
