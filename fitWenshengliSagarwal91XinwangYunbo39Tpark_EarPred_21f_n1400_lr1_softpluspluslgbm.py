...
def get_df_feat(data, flatten_mask):
    ...
    ####### feature 5: 1-d feature, ret1_spx
    ...
    ####### feature 6: analyst feature
    var_name = 'bbganalyst'
    var = op.ts_fill(data['load_simvar_hhmm'](var_name))
    var = op.ts_zscore(var,21)
    df[var_name] = op.at_nan2zero(var[flatten_mask])

    ####### feature 7: spillover - in previous 5 days, whether there exists stocks in the same industry has announced earnings, and jump of price
    ...
    #### feature 8: shortint_v3 - modified from GAR-team 4 feature
    SIR_v2 = data['load_simvar_hhmm']('tq_shortintratio_abnormal_v1_v20')
    SIR_v2 = op.ts_fill(SIR_v2)
    SIR_v2 = op.at_nan2zero(SIR_v2)
    df['shortint_v3'] = SIR_v2[flatten_mask]
...
def custom_loss_function(y_true, y_pred):
    grad = y_true*(y_pred*y_true - (np.exp(y_pred*y_true) + 1)*np.log((np.exp(y_pred*y_true) + 1)*np.exp(-y_pred*y_true)) + 1)/(np.exp(y_pred*y_true) + 1)
    hess = y_true**2*(y_pred*y_true + (-y_pred*y_true + 1)*(np.exp(y_pred*y_true) + 1) + 1)/(np.exp(y_pred*y_true) + 1)**2
    return grad, hess
...
def fit_model(feat_X, Y):
    ...
    model = lgb.LGBMRegressor(objective=custom_loss_function, random_state=42, n_estimators=1800, learning_rate=0.06, num_leaves=2, num_threads=-1, colsample_bytree=0.75)
    model.fit(feat_X, Y)
