...
def gradient(predt: np.ndarray, dtrain: xgb.DMatrix):
    ...
    y_pred = predt
    return -y_true / (np.exp(y_pred * y_true) + 1)
...
def hessian(predt: np.ndarray, dtrain: xgb.DMatrix):
    ...
    y_true = dtrain.get_label()
    y_pred = predt
    return y_true ** 2 / (4 * np.cosh(y_pred * y_true / 2) ** 2)


def squared_softplus(predt: np.ndarray,
                     dtrain: xgb.DMatrix):
...
def fit_model(feat_X, Y):
    ...
    dtrain = xgb.DMatrix(feat_X, label=Y)
    bst = xgb.train(
        {
            'eta': 0.1,
            'max_depth': 1,
