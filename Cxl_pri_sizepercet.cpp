...
void OnSnapshot(float g_snapshot_time = 34200.10) {
    ...
    // 获取B,S 研究对象集合中订单的size_percentage=order.size/level_size(order 所在level) 计算size_perc_mean 并返回size_perc_mean_B, size_perc_mean_S, num_B, num_S
    // 找出B,S 研究对象集合中订单的size_percentage在区间[0, 0.2, 0.4, 0.7, 1.0] 的订单数量占B/S 订单数量的百分比，满足这些区间的订单amt(size*price)占总B/S 分别amt的比例，size_perc_mean每个区间订单的均值,分别返回这些B/S 的值
